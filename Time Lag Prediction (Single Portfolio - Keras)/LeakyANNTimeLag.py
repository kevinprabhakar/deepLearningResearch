...
from keras.layers import Input, Dense, LSTM, TimeDistributed, Dropout, Flatten, LeakyReLU
...
def createTimeLag(dataSet, rownum):
    dataX = []
    dataY = []
...
x,y = createTimeLag(tenByTenData, 0)
...
forecastInputs = Input(shape=(exampleNum,timeWindow))
forecasterFlatten = Flatten()(forecastInputs)
forecasterHidden1 = Dense(lookForwardPeriod*2, activation='linear')(forecasterFlatten)
forecasterLeaky1 = LeakyReLU()(forecasterHidden1)
forecastOutputs = Dense(lookForwardPeriod, activation='linear')(forecasterLeaky1)

forecaster = Model(forecastInputs,forecastOutputs)
forecaster.compile(optimizer=Nadam(lr=0.002), loss='mse', metrics=['accuracy'])


hist = forecaster.fit(xTrain,yTrain,epochs=500, shuffle=True)
#
